    ...
    return function(target: any, propKey: PropertyKey, propDescriptor: PropertyDescriptor) {
        function addWatch(func: AsyncFunction) {
            return async function(req: Request, res: Response) {
                    console.log(`[${chalk.green(new Date().toLocaleString())}][${chalk.bold(req.method)} ${chalk.yellow(`${req.baseUrl}${req.path}`)}]`);
                if (options?.deepWatch) {
                    console.log('Headers:');
                    Object.keys(req.headers).forEach(key => {
                        console.log(`\t${chalk.yellow(key)}: ${chalk.blue(JSON.stringify(req.headers[key]))}`);
                    });
                    console.log('Query:');
                    Object.keys(req.query).forEach(key => {
                        console.log(`\t${chalk.yellow(key)}: ${chalk.blue(JSON.stringify(req.query[key]))}`);
                    });
                    console.log('Params:');
                    Object.keys(req.params).forEach(key => {
                        console.log(`\t${chalk.yellow(key)}: ${chalk.blue(JSON.stringify(req.params[key]))}`);
                    });
                    console.log('Body:');
                    Object.keys(req.body).forEach(key => {
                        console.log(`\t${chalk.yellow(key)}: ${chalk.blue(JSON.stringify(req.body[key]))}`);
                    });
                }
                try {
                    await func(req, res);
                    ...
                    console.error(err);
                    if (!res.headersSent) res.status(HttpStatusCode.INTERNAL_SERVER_ERROR).send({ message: 'Server error!' });
                }
            }
        }
            ...
            return async function(req: Request, res: Response) {
                ...
                options?.query?.forEach(ele => {
                    if (!queryKeys.includes(ele.name)) {
                        ...
                        ok = false;
                    } else if (ele.type && req.query[ele.name]?.constructor.name != ele.type.name) {
                        err.push({
                            at: ele.name,
                        ...
                        const msg = ele.validator ? ele.validator(ele.name, req.query[ele.name]) : null;
                        if (msg) {
                            err.push({
                                at: ele.name,
                                message: msg
                ...
                options?.params?.forEach(ele => {
                    if (!paramKeys.includes(ele.name)) {
                        ...
                        ok = false;
                    } else if (ele.type && req.params[ele.name]?.constructor.name != ele.type.name) {
                        err.push({
                            at: ele.name,
                            message: `${ele.name} must be ${ele.type.name}.`
                        });
                        ...
                        const msg = ele.validator ? ele.validator(ele.name, req.params[ele.name]) : null;
                        if (msg) {
                            err.push({
                                at: ele.name,
                                message: msg
                ...
                options?.body?.forEach(ele => {
                    if (!bodyKeys.includes(ele.name)) {
                        ...
                        ok = false;
                    } else if (ele.type && req.body[ele.name]?.constructor.name != ele.type.name) {
                        err.push({
                            at: ele.name,
                            message: `${ele.name} must be ${ele.type.name}.`
                        });
                        ...
                        const msg = ele.validator ? ele.validator(ele.name, req.body[ele.name]) : null;
                        if (msg) {
                            err.push({
                                at: ele.name,
                                message: msg
