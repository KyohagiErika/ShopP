...
class AuthMiddleware {
  ...
  static async loginWithEmail(req: Request, res: Response) {
    ...
    if (result.getCode() === HttpStatusCode.OK) {
      ...
      res.status(result.getCode()).send({ message: result.getMessage(), token: result.getData() });
    } else {
  ...
  static async changePassword(req: Request, res: Response) {
    ...
    const result = await AuthModel.changePassword(id, data.oldPassword, data.newPassword);
    res.status(result.getCode()).send({ message: result.getMessage() });
  }
}
