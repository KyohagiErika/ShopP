...
export default class AuthModel {
    static async loginWithEmail(email: string, password: string) {
        ...
        if (user !== null) {
            ...
            if (!user.checkIfUnencryptedPasswordIsValid(password)) {
                return new Response(HttpStatusCode.UNAUTHORIZATION, 'Wrong login password');
            }
        ...
        } else return new Response(HttpStatusCode.BAD_REQUEST, 'Wrong login email!');
    }
    ...
    static async changePassword(id: number, oldPassword: string, newPassword: string) {
        ...
        } else return new Response(HttpStatusCode.UNAUTHORIZATION, 'Unauthorized error, user not exist!');
    }
}